...
**Note**: Axer is currently in alpha (`test-1`) and is not considered stable. It supports Android, JVM, and iOS platforms. However, iOS has limitations: stack traces and fatal crash capturing via `installAxerErrorHandler` are not supported.
...
- **Stability**: The library is in alpha (`test-1`) and may have bugs or breaking changes in future releases.
