...
**Note**: Axer is currently in alpha (`1.0.0-alpha25`) and is not considered stable. It supports Android, JVM, and iOS platforms. However, iOS has limitations: stack traces and fatal crash capturing via `installAxerErrorHandler` are not supported.
...
```kotlin
implementation("io.github.orioneee:axer:1.0.0-alpha25")
```

For production environments where monitoring is not needed, use the no-op variant to avoid code changes:

```kotlin
implementation("io.github.orioneee:axer-no-op:1.0.0-alpha25")
```
...
- **Stability**: The library is in alpha (`1.0.0-alpha25`) and may have bugs or breaking changes in future releases.
