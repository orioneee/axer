...
```kotlin
implementation("io.github.orioneee:axer:1.0.0-RC3")
```

For production environments where monitoring is not needed, use the no-op variant to avoid code changes:

```kotlin
implementation("io.github.orioneee:axer-no-op:1.0.0-RC3")
```
...
The library is in beta (`1.0.0-RC3`) and may have bugs or breaking changes in future releases.
